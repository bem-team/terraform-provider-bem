...
func noConfiguredAttributeChanged(
	...
	for name, attribute := range rschema.Attributes {
		...
		if !equal {
			...
			logValueDiff(ctx, "bem_function", name, realDiffs)
			return false, diags
		}
	}
...
//
// On bem_function that attribute is `function`, the read-only mirror of the API
// response. Measured from a real plan's JSON, its `after_unknown` contained
...
func preserveServerDefaults(
	...
	return tftypes.Transform(plan.Raw, func(p *tftypes.AttributePath, v tftypes.Value) (tftypes.Value, error) {
		...
		// config must adopt that null, or the guard diffs unknown-vs-null and the
		// plan never settles. This matches the framework's own UseStateForUnknown,
		// which documents that null is a known value and is copied to the planned
		// value.
		//
		// Adopted for PRIMITIVES only, which is what makes it safe. The attributes
		// that need it are the ones the API assigns per function type:
		// pre_count / tabular_chunking_enabled / enable_bounding_boxes are returned
		// for an extract function and absent for every other type. Being
		// Optional+Computed (they have to be - the server assigns them, and a
		// plain-Optional attribute cannot be planned back to a server value at all,
		// see the audit test) they are marked unknown when the configuration omits
		// them, so on a non-extract function the plan holds unknown against a null
		// state and churns forever. Caught by
		// TestAccFunctionResource_EnrichReplansClean.
		//
		// For objects, lists and maps the bail stands, conservatively. Adopting a
		// null there once produced an apply-time failure:
		//
		//	.config: was null, but now cty.ObjectVal{endpoints: null, steps: null}
		//
		// caught by TestAccFunctionResource_ImportThenUpdate step 2.
		//
		// The cause is NOT established, and two plausible explanations have been
		// tested and ruled out - do not repeat them:
		//
		//   - the decoder: hydrateFromResponse keeps a null config null;
		//   - the round trip through req.Plan.Get / resp.State.Set on a null
		//     customfield.NestedObject. Both directions preserve the null. An
		//     earlier version of this comment asserted this was the cause; it is
		//     wrong.
		//
		// The remaining hypothesis is tftypes.Transform rebuilding a null parent
		// when a descendant is substituted - which hasNullAncestor (below) was
		// added to prevent, and which did not exist when that failure was seen. So
		// objects may well be safe now. Deciding it needs one acceptance run with
		// the isPrimitiveType gate removed; the guards that would catch a
		// regression are TestAccFunctionResource_{ImportThenUpdate,
		// ImportMinimalPlansClean,EnrichReplansClean} and
		// TestPreserveServerDefaults_LeavesNullParentNull.
		//
		// Until then the gate stays: the only shapes it costs are a
		// collection-only enrich function with no `endpoints`, and parse_config
		// (see the audit test).
...
// be "materialised" into anything.
//
// The distinction matters to preserveServerDefaults. Adopting a null prior state
// for a scalar simply plans null. Doing it for an object, list or map trips a
// separate defect on the Update path, where a null nested object comes back as a
// known object full of nulls and fails the plan/apply consistency check.
